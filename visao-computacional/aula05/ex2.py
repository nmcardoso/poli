...
def main():
  ax = cv2.imread("aprendizagem/janei.pgm", 0)
  ay = cv2.imread("aprendizagem/janei-1.pgm", 0)
  
  if ax.shape[0] != ay.shape[0] or ax.shape[1] != ay.shape[1]:
    sys.exit("Erro: Imagens com dimensoes diferentes")
    
  features = preprocess_features(ax/255, 3)
  ...
  qx = cv2.imread("aprendizagem/julho.pgm", 0)
  query = preprocess_features(qx/255, 3)
  
  qp = arvore.predict(query)
  qp = qp.reshape((qx.shape[0], qx.shape[1]))

  qp_filtered = cv2.medianBlur(np.float32(qp), 5)
  qp_filtered = 1 - qp_filtered
  qp_mask = qp_filtered == 1
  
  final = cv2.cvtColor(qx, cv2.COLOR_GRAY2RGB)
  final[qp_mask, 2] = 255
  
  w = 350
  h = 450
  show('Treino', ax, width=w, height=h, x=10, y=30)
  show('Referencia', ay, width=w, height=h, x=400, y=30)
  show('Query', qx, width=w, height=h, x=10, y=550)
  show('Resultado', qp, width=w, height=h, x=400, y=550)
  show('Resultado filtrado e invertido', qp_filtered, width=w, height=h, x=800, y=550)
  show('Resultado final', final, width=w, height=h, x=1200, y=550)
  cv2.waitKey(0)
